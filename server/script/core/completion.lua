...
local function tryType(ast, text, offset, results)
    ...
    if source then
        if source.parent.type == "type.module" then
            ...
            local builtIn = {}
            for _, lib in ipairs({api.object, defaultlibs.customType}) do
...
local function getEnumDescription(enum)
    ...
    if enum.descLocation.class then
        local object = api.object[enum.descLocation.class]
        if object then
            return api.object[enum.descLocation.class].description
        end
    elseif enum.descLocation.enum then
        for _, enumParent in ipairs(api.object["Enums"].child) do
