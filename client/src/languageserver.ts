...
function start(context: ExtensionContext, documentSelector: DocumentSelector, folder: WorkspaceFolder) {
    ...
    let develop: boolean = config.get("miliastraLsp.develop.enable");
    let debuggerPort: number = config.get("miliastraLsp.develop.debuggerPort");
    let debuggerWait: boolean = config.get("miliastraLsp.develop.debuggerWait");
    let commandParam: string = config.get("miliastraLsp.misc.parameters");
    let command: string;
